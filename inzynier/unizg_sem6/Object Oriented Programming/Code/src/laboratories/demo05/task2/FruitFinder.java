...
public class FruitFinder {
     ...
     */
    public static Predicate<Fruit> allFruitThatMatchColorAndPattern(FruitColor color, String pattern) {       
        return f -> (f.getColor() == color && f.getName().contains(pattern));
    }
     ...
     * @return the {@link Predicate} instance
     */
    public static Predicate<Fruit> allFruitThatDoesNotMatchColorAndDoesNotContainPattern(FruitColor color, String pattern) {
        return f -> (f.getColor() != color && !f.getName().contains(pattern));
    }
}
